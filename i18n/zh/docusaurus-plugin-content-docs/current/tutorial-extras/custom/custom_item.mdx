...
- `legacyBuilder` Legacy 模式物品构建器（服务端仅注册标识符，贴图由资源包提供）

- `legacyFoodBuilder` Legacy 模式食物构建器
...
- `creativeGroup(String creativeGroup)` 控制自定义物品在创造栏的分组，使用字符串标识符，例如 `"itemGroup.name.sword"`。常用值请参考 [ItemCreativeGroup 参考](#itemcreativegroup-key-methods)。

- `tag(String... tags)` 为自定义物品添加一个或多个标签。

- `canDestroyInCreative(boolean value)` 控制拿该物品的玩家是否可以在创造模式挖掘方块。

### ItemCreativeGroup 参考 \{#itemcreativegroup-key-methods}
...
`ItemCreativeGroup` 枚举已被标注为废弃（`@Deprecated`）。推荐直接使用字符串值，例如：`.creativeGroup("itemGroup.name.sword")`。
...
### 自定义物品类型 \{#custom-item-types}

Nukkit-MOT 提供了多个基类用于不同类型的自定义物品：

| 类名 | 说明 |
|---|---|
| `ItemCustom` | 基础自定义物品 |
| `ItemCustomTool` | 自定义工具（镐、斧、铲等） |
| `ItemCustomArmor` | 自定义盔甲（头盔、胸甲、护腿、靴子） |
| `ItemCustomEdible` | 自定义食物 |
| `ItemCustomProjectile` | 自定义投掷物 |
| `ItemCustomBookEnchanted` | 自定义附魔书 |

根据你要创建的物品类型选择合适的基类，每个类都提供了针对其物品类型的特定方法和行为。

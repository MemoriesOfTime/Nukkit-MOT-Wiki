...
- Media (game traffic) over **UDP** — shares `server-port`'s UDP side (`19132`) with RakNet by default, relayed in-process from the RakNet socket, so no extra port is needed; pin a standalone media port (e.g. `19134`), a port window, or `0` for system-assigned ports via [`server-udp-ports`](server-properties.mdx#server-udp-ports), which also explains what to do when media UDP is unreachable

<DefaultPortsGrid />

Monitoring: `/status` prints a NetherNet block covering the last 10 minutes (joins accepted, ICE connections, completed handshakes); `/status full` adds media traffic, drop counters and shared-port relay statistics. The block is hidden while NetherNet is disabled.
...
While enabled, signaling shares `server-port` over TCP and media shares its UDP side by default (relayed in-process from the RakNet socket — pin a standalone port like `19134` via [`server-udp-ports`](server-properties.mdx#server-udp-ports) if you prefer; `0` reverts media to system-assigned ports)
...
maximum number of blocks a single piston can move (1-64)

values outside the 1-64 range fall back to 12; bedrock edition uses 12

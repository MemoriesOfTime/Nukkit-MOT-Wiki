...
- `legacyBuilder` - A Legacy mode item builder (server only registers the identifier, textures are provided by the resource pack).
- `legacyFoodBuilder` - A Legacy mode food builder.
...
- `creativeGroup(String creativeGroup)` - Controls the custom item's group in the creative inventory using a string identifier, such as `"itemGroup.name.sword"`. See the [ItemCreativeGroup reference](#itemcreativegroup-key-methods) for common values.
- `tag(String... tags)` - Adds one or more tags to the custom item.
- `canDestroyInCreative(boolean value)` - Controls whether a player holding the item can break blocks in creative mode.

### ItemCreativeGroup Reference \{#itemcreativegroup-key-methods}
...
The `ItemCreativeGroup` enum is deprecated (`@Deprecated`). It is recommended to use string values directly, for example: `.creativeGroup("itemGroup.name.sword")`.

:::

Common creative group string values:
...
### Custom Item Types \{#custom-item-types}

Nukkit-MOT provides several base classes for different types of custom items:

| Class | Description |
|---|---|
| `ItemCustom` | Base custom item class |
| `ItemCustomTool` | Custom tool (pickaxe, axe, shovel, etc.) |
| `ItemCustomArmor` | Custom armor (helmet, chestplate, leggings, boots) |
| `ItemCustomEdible` | Custom food item |
| `ItemCustomProjectile` | Custom throwable/projectile item |
| `ItemCustomBookEnchanted` | Custom enchanted book |

Choose the appropriate base class based on the type of item you're creating. Each class provides specific methods and behaviors tailored to its item type.

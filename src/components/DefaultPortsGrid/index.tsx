 ...
 * server-udp-ports=19132): TCP+UDP 19132 are listening — the UDP side serves
 * RakNet and the relayed NetherNet media — the rest of the 1913x row stays free.
 */
const DefaultPortsGrid: React.FC<Props> = ({
    labelTcp = 'TCP',
    labelUdp = 'UDP',
    labelSignaling = 'NetherNet signaling',
    labelRaknet = 'RakNet + NetherNet media (relayed)',
    labelIpv6Idle = 'IPv6 listener (off by default)',
    legendActive = 'listening by default',
    legendIdle = 'not used by default',
    caption = 'Port usage with the default configuration (server-port=19132, NetherNet enabled, server-udp-ports=19132: media shares the RakNet port)',
}) => (
